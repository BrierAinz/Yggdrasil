...
## Dark Fantasy Nordic Tech Template

For the user's BrierStudios brand and Yggdrasil ecosystem aesthetic, a dedicated design system exists:

- **Reference**: `references/dark-fantasy-nordic-design.md` — full frost palette, typography, rune reference, animation keyframes, logo remastering technique, responsive breakpoints

The aesthetic uses a **frost/ice palette** (NOT gold — the user explicitly requested "mas frio"). Key features: cold void backgrounds (#060810), ice-blue accents (#38bdf8), aurora violet secondary (#818cf8), Elder Futhark rune decorations, floating rune particle canvas, mouse-following ice glow, frost trail particles, frosted-glass navbar, glow-on-hover cards, loading screen with rune pulse, Cinzel serif headings, JetBrains Mono tech tags. The user prefers **maximum animation and movement** — always add 2-3+ dynamic elements.
