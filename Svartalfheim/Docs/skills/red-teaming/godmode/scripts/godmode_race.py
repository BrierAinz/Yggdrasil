...
REFUSAL_PATTERNS = [
    ...
    re.compile(
        r"(?:apologize|sorry),? (?:but )?(?:I |this |that )(?:cannot|can't|isn't|won't)", re.I
    ),
    ...
    re.compile(
        r"\bI'm\s+not\s+(?:comfortable|able|permitted)\s+(?:to\s+)?(?:provid|shar|giv|help)", re.I
    ),
]
...
HEDGE_PATTERNS = [
    ...
    re.compile(
        r"consult (?:a |with |your )?(?:professional|doctor|lawyer|expert|specialist)", re.I
    ),
