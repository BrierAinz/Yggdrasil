...
class ArxivSearchTool:
    ...
    async def is_available(self) -> bool:
        ...
        try:
            ...
            resp = await client.get(self.ARXIV_API, params={"max_results": 1, "search_query": "all:electron"})
...
class SourceAnalyzer:
    ...
    def _compute_score(self, source: Dict[str, Any], topic_terms: set) -> float:
        ...
        text = f"{source.get('title', '')} {source.get('snippet', source.get('abstract', ''))}".lower()
...
class ReportGenerator:
    ...
    def generate(
        ...
        lines.append("")
        lines.append(f"> **{depth_label}** — Generated by Mimir on {now.strftime('%Y-%m-%d %H:%M UTC')}")
        lines.append(f"> *The Well of Wisdom draws from {len(sources)} sources and {len(arxiv_papers)} academic papers.*")
        lines.append("")
        ...
        # Footer
        lines.append("---")
        lines.append(f"*Report generated by Mimir — The Well of Wisdom*")
        ...
        if output_dir:
            ...
            slug = MimirAgent._slugify(topic) if False else self._slugify(topic)
    ...
    def _generate_summary(self, topic: str, sources: List, papers: List) -> str:
        ...
        if snippets:
            ...
            return f"Research on **{topic}** reveals insights from {len(sources)} web sources and {len(papers)} academic papers. {combined}"
        else:
            return f"Research on **{topic}** examined {len(sources)} web sources and {len(papers)} academic papers, though detailed summaries were not available."
    ...
    def _generate_findings(self, sources: List, papers: List) -> List[str]:
        ...
        for paper in papers[:3]:
            ...
            if title and not any(title in f for f in findings):
                findings.append(f"Academic: **{title}** — peer-reviewed research relevant to this topic.")
