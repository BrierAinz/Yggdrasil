...
if "error" not in result:
    print(
        f"   [OK] Online - v{result.get('version', '?')} - Tools: {result.get('tools_registered', 0)}"
    )
else:
...
result = test_endpoint(
    "docs", "http://localhost:8000/api/docs/query", method="POST", data=data, timeout=30
)
if "error" not in result and result.get("sources"):
    print(
        f"   [OK] RAG funciona - Fuentes: {len(result['sources'])}, Conf: {result.get('confidence', 0):.0%}"
    )
else:
