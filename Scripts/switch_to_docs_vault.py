...
AGENT_SCRIPT = Path(
    "D:/Proyectos/Yggdrasil/Asgard/Lilith/Core/Backend/core/agents/archivero_agent.py"
)
...
def update_scripts():
    ...
    # 1. Indexador
    with open(INDEXER_SCRIPT, encoding="utf-8") as f:
        content = f.read()

    content = content.replace(
        'VAULT_NAME = "default"  # Fallback', 'VAULT_NAME = "docs"  # Vault oficial del Archivero'
    )
    ...
    # 2. API
    with open(API_SCRIPT, encoding="utf-8") as f:
        content = f.read()

    content = content.replace(
        'VAULT_NAME = "default"  # Cambiar a "docs"', 'VAULT_NAME = "docs"  # Vault oficial'
    )
    ...
    if test_script.exists():
        ...
        content = content.replace(
            '"vault": "default",  # cambiar a "docs"', '"vault": "docs",  # Vault oficial'
        )
