...
function getOrCreatePanel(ctx: vscode.ExtensionContext): vscode.WebviewPanel {
  if (panel) {
...
function buildHtml(
  ...
  prompt: string,
  response: string,
  meta: { workspaceName?: string; filePath?: string; languageId?: string }
): string {
...
async function callLilithAPI(payload: {
  ...
  const token = (cfg.get<string>('vscodeToken', '') || '').trim()
  ...
  try {
    ...
    const result = await postJson<AskResponse>(`${serverUrl}/api/vscode/ask`, payload, headers)
...
async function runAskCommand(ctx: vscode.ExtensionContext): Promise<void> {
...
export function activate(ctx: vscode.ExtensionContext): void {
  ctx.subscriptions.push(
    vscode.commands.registerCommand('lilith.ask', () => runAskCommand(ctx))
  )
  ...
  const applyPatch = vscode.commands.registerCommand('lilith.applyPatch', async () => {
    ...
    const prompt = await vscode.window.showInputBox({
      prompt:
        'Instrucción para Lilith (ej: "refactoriza", "añade tipos", "corrige el bug")',
    ...
    await vscode.window.withProgress(
      ...
      async () => {
        ...
        const choice = await vscode.window.showInformationMessage(
          '¿Aplicar este parche sobre tu selección?',
          { modal: true },
          'Aplicar',
          'Cancelar'
        )
